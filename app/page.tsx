...
export default function Home() {
 return <div className="min-h-screen bg-[#fbfcfa] text-[#18304a]"><Header/><main>
  ...
  <section className="mx-auto max-w-[1180px] px-5 py-8 sm:px-8 sm:py-10"><div className="mb-5 flex items-end justify-between"><div><span className="text-xs font-black uppercase tracking-[.16em] text-[#168b50]">Choose your learning mode</span><h2 className="mt-2 text-3xl font-black tracking-[-.04em] sm:text-4xl">Learning designed around you.</h2></div></div>
   <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{cards.map((c,i)=><Link href={i===1?'/tutors':'/subjects'} key={c.title} className="group overflow-hidden rounded-[24px] border border-[#d9e5df] bg-[#f3f5f4] shadow-[0_8px_28px_rgba(26,55,42,.07)]"><div className="overflow-hidden"><img src={c.image} alt="" className="h-48 w-full object-cover transition duration-500 group-hover:scale-105 sm:h-52"/></div><div className="p-5"><div className="flex items-center justify-between"><h3 className="text-xl font-black">{c.title}</h3><span className="grid h-9 w-9 place-items-center rounded-full bg-[#dff3e7] text-lg font-black text-[#168b50]">→</span></div><p className="mt-2 text-sm leading-6 text-[#617185]">{c.text}</p></div></Link>)}</div>
  </section>

  <section className="bg-[#eaf5ff] py-12 sm:py-14"><div className="mx-auto max-w-[1180px] px-5 sm:px-8"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><span className="text-xs font-black uppercase tracking-[.16em] text-[#2876c6]">Competitive preparation</span><h2 className="mt-2 text-3xl font-black tracking-[-.04em] sm:text-4xl">Prepare for your next goal.</h2><p className="mt-2 text-[#53677b]">Experienced faculty and structured subject support.</p></div><Link href="/subjects" className="font-extrabold text-[#168b50]">View all subjects →</Link></div><div className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-5 lg:grid-cols-10">{exams.map((e,i)=><Link href="/subjects" key={e} className="rounded-2xl border border-[#c9dce9] bg-[#f2f5f6] px-3 py-5 text-center shadow-sm transition hover:-translate-y-1"><span className="text-3xl font-black text-[#168b50]">{String(i+1).padStart(2,'0')}</span><div className="mt-2 text-xs font-black leading-4">{e}</div></Link>)}</div></div></section>
  ...
  <section className="border-y border-[#d8e1dd] bg-[#eef1f0]"><div className="mx-auto grid max-w-[1180px] grid-cols-2 sm:grid-cols-4">{[['Experienced Tutors','Expert faculty selected for the learning requirement.'],['Personalised Learning','Support shaped around the student’s needs.'],['Regular Parent Updates','Clear class, attendance and progress updates.'],['Managed Learning Journey','TutorVerse coordinates the learning experience.']].map(([t,d],i)=><div key={t} className={`px-5 py-8 ${i?'border-l border-[#d4ddd9]':''}`}><div className="text-xl font-black leading-tight text-[#183a5b]">{t}</div><p className="mt-2 text-xs font-semibold leading-5 text-[#667487]">{d}</p></div>)}</div></section>
 </main><Footer/></div>
}
