...
const modes = [
  { title: 'Batch Tuition', text: 'Structured learning. Stronger concepts.', image: 'https://images.unsplash.com/photo-1509062522246-3755977927d7?auto=format&fit=crop&w=1400&q=90', icon: 'B', tone: 'bg-[#dff5e8]', accent: 'text-[#168b50]' },
  { title: 'Home Tuition', text: 'One-to-one attention at your home.', image: 'https://images.unsplash.com/photo-1544717305-2782549b5136?auto=format&fit=crop&w=1400&q=90', icon: 'H', tone: 'bg-[#e1f2fb]', accent: 'text-[#1466b8]' },
  { title: 'Online Tuition', text: 'Learn from anywhere. Same great quality.', image: 'https://images.unsplash.com/photo-1529390079861-591de354faf5?auto=format&fit=crop&w=1400&q=90', icon: 'O', tone: 'bg-[#f5e5f8]', accent: 'text-[#7041c8]' },
  { title: 'For Students & Parents', text: 'Track progress and get regular updates.', image: 'https://images.unsplash.com/photo-1587654780291-39c9404d746b?auto=format&fit=crop&w=1400&q=90', icon: 'P', tone: 'bg-[#fff1d8]', accent: 'text-[#ec8314]' },
];
...
export default function Home() {
  return <div className="min-h-screen bg-white text-[#18304a]"><Header/><main>
    <section className="border-b border-[#e9eeeb] bg-[#f7fbf8]">
      <div className="mx-auto max-w-[1180px] px-5 py-6 sm:px-8 sm:py-8">
        <div className="grid items-center overflow-hidden rounded-[28px] border border-[#dfece4] bg-white shadow-[0_10px_35px_rgba(27,67,48,.07)] lg:grid-cols-[1.05fr_.95fr]">
          ...
          <div className="relative h-[270px] overflow-hidden sm:h-[310px] lg:h-[360px]"><img src="https://images.unsplash.com/photo-1503676260728-1c00da094a0b?auto=format&fit=crop&w=1400&q=90" alt="Student learning" className="h-full w-full object-cover"/><div className="absolute left-5 top-5 rounded-2xl bg-white/95 px-5 py-4 shadow-xl"><div className="text-base font-black leading-tight text-[#183a5b]">Learn.<br/><span className="text-[#168b50]">Practice. Grow.</span></div><div className="mt-2 h-1 w-12 rounded-full bg-[#f08a19]"/></div><div className="absolute bottom-5 right-5 rounded-2xl bg-[#183a5b] px-4 py-3 text-sm font-extrabold text-white shadow-xl">Personal. Simple. Managed.</div></div>
        </div>
      </div>
    </section>
    ...
    <section className="mx-auto max-w-[1180px] px-5 py-8 sm:px-8 sm:py-10">
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{modes.map((m)=><Link href={m.title==='Home Tuition'?'/tutors':'/subjects'} key={m.title} className={`group overflow-hidden rounded-[24px] border border-[#e2e9e5] ${m.tone} shadow-[0_8px_28px_rgba(26,55,42,.07)]`}><div className="h-[205px] overflow-hidden sm:h-[220px]"><img src={m.image} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105"/></div><div className="px-5 pb-5 pt-0 text-center"><div className={`mx-auto -mt-7 grid h-14 w-14 place-items-center rounded-full bg-white text-xl font-black shadow-md ring-4 ring-white/80 ${m.accent}`}>{m.icon}</div><h3 className={`mt-3 text-[19px] font-black ${m.accent}`}>{m.title}</h3><p className="mt-1 text-sm leading-5 text-[#263b4f]">{m.text}</p><div className={`mt-3 text-lg font-black ${m.accent}`}>→</div></div></Link>)}</div>
    </section>
