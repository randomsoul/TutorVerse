...
function clean(value: unknown, max = 100) { return typeof value === "string" ? value.trim().slice(0, max) : ""; }
...
Deno.serve(async (req: Request) => {
  ...
  let body: { action?: string; role?: string; profile_id?: string; new_password?: string };
  ...
  if (action === "change-password") {
    ...
    if (caller.id === profileId) return json({ error: "You cannot change your own password here. Use Forgot password or account settings." }, 400);
    ...
    const { error: passwordError } = await adminClient.auth.admin.updateUserById(profileId, { password: newPassword });
  ...
  if (action === "list") {
    ...
    for (const row of rows || []) {
      ...
      if (pid) {
        ...
        profile = p || null;
        const { data: authData } = await adminClient.auth.admin.getUserById(pid);
  ...
  if (action === "delete") {
    ...
    if (roleDeleteError) return json({ error: `This ${role} cannot be permanently deleted because related records still depend on it. Use Deactivate/Archive instead. Details: ${roleDeleteError.message}` }, 409);
